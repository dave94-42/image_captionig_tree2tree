...
def predict_test_dataSet(decoder, encoder, input_test, target_test, test_data, file_name):
    global s
    predicted = []
    batch_enc = encoder(input_test)
    batch_dec = decoder(encodings=batch_enc.get_root_embeddings(), targets=target_test)
    i=0
    for tree_dec, tree in zip(batch_dec.decoded_trees, test_data):
        get_predict_embeddings(tree_dec)
        predicted.append({'image_id': tree['name'],
                          'caption': s})
        s = ""
        i+=1
