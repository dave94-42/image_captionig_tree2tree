...
def load_data(args,tree_encoder,tree_decoder,tree_cnn_type):
    ...
    train_data = read_images(args[0],image_features_extract_model,tree_cnn_type)
    val_data = read_images(args[1],image_features_extract_model,tree_cnn_type)
    print('loading sentence trees...')
    if tree_decoder:
        label_tree_with_sentenceTree(train_data,val_data, args[2])
    val_all_captions = load_all_captions(args[3],val_data)
